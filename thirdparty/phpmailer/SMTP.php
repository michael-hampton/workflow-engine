...
class SMTP
{
    ...
    public function Connect ($host, $port = 0, $tval = 30)
    {
        ...
        {
            $this->error = array("error" => 'ID_SMTP_FAILED_CONNECT_SERVER',
                "errno" => $errno,
                "errstr" => $errstr);
            if ( $this->do_debug >= 1 )
            {
    ...
    public function StartTLS ()
    {
        ...
        if ( $code != 220 )
        {
            $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_START_TLS_NOT_ACCEPTED'),
                        "smtp_code" => $code,
    ...
    public function Authenticate ($username, $password, $authtype = 'LOGIN', $realm = '', $workstation = '')
    {
        ...
        switch ($authtype) {
            case 'PLAIN':
                ...
                if ( $code != 334 )
                {
                    $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_AUTH_NOT_ACCEPTED'),
                                "smtp_code" => $code,
                                "smtp_msg" => substr ($rply, 4));
                    if ( $this->do_debug >= 1 )
                    {
            ...
            case 'LOGIN':
                ...
                if ( $code != 334 )
                {
                    $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_USERNAME_NOT_ACCEPTED'),
                                "smtp_code" => $code,
                                "smtp_msg" => substr ($rply, 4));
                    if ( $this->do_debug >= 1 )
                    {
                ...
                if ( $code != 235 )
                {
                    $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_PASSWORD_NOT_ACCEPTED'),
                                "smtp_code" => $code,
                                "smtp_msg" => substr ($rply, 4));
                    if ( $this->do_debug >= 1 )
                    {
            ...
            case 'NTLM':
                ...
                if ( $code != 235 )
                {
                    $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_COULD_NOT_AUTH'),
                                "smtp_code" => $code,
                                "smtp_msg" => substr ($rply, 4));
                    if ( $this->do_debug >= 1 )
                    {
    ...
    public function Data ($msg_data)
    {
        ...
        if ( $code != 354 )
        {
            $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_DATA_COM_NOT_ACCEPTED'),
                        "smtp_code" => $code,
                        "smtp_msg" => substr ($rply, 4));
            if ( $this->do_debug >= 1 )
            {
        ...
        if ( $code != 250 )
        {
            $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_DATA_NOT_ACCEPTED'),
                        "smtp_code" => $code,
                        "smtp_msg" => substr ($rply, 4));
            if ( $this->do_debug >= 1 )
            {
    ...
    private function SendHello ($hello, $host)
    {
        ...
        if ( $code != 250 )
        {
            $this->error = array("error" => $hello . ' ' . G::LoadTranslation ('ID_SMTP_ERROR_NOT_ACCEPTED'),
                        "smtp_code" => $code,
                        "smtp_msg" => substr ($rply, 4));
            if ( $this->do_debug >= 1 )
            {
    ...
    public function Mail ($from)
    {
        ...
        if ( $code != 250 )
        {
            $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_MAIL_NOT_ACCEPTED'),
                        "smtp_code" => $code,
                        "smtp_msg" => substr ($rply, 4));
            if ( $this->do_debug >= 1 )
            {
    ...
    public function Recipient ($to)
    {
        ...
        {
            $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_RCPT_NOT_ACCEPTED'),
                        "smtp_code" => $code,
                        "smtp_msg" => substr ($rply, 4));
            if ( $this->do_debug >= 1 )
            {
    ...
    public function Reset ()
    {
        ...
        {
            $this->error = array(
                "error" => 'ID_SMTP_ERROR_CALLED_RESET_WITHOUT');
            return false;
        }
        ...
        if ( $code != 250 )
        {
            $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_RSET_FAILED'),
                        "smtp_code" => $code,
                        "smtp_msg" => substr ($rply, 4));
            if ( $this->do_debug >= 1 )
            {
    ...
    public function SendAndMail ($from)
    {
        ...
        if ( $code != 250 )
        {
            $this->error = array("error" => G::LoadTranslation ('ID_SMTP_ERROR_SAML_NOT_ACCEPTED'),
                        "smtp_code" => $code,
                        "smtp_msg" => substr ($rply, 4));
            if ( $this->do_debug >= 1 )
            {

...
class Dirs
{
        ...
        function getFilesData ($path)
        {
            ...
            foreach ($tmp as $ff) {
                ...
                else if ( is_file ($path . '/' . $ff) )
                {
                    $files++;
                    if ( $path == MAIN_ROOT )
                        Dirs::$dir_tree['files'][] = Dirs::setFileData ($path . '/' . $ff);
                }
    ...
    static public function createDir ($path, $name)
    {
        if ( is_dir ($path) )
        {
            if ( mkdir ($path . '/' . $name, octdec (DIRPERMISSIONS)) )
            {
                (new AppFolder())->createFromPath ($name);
                $re = 'ok';
            }
            else
            {
    ...
    static public function deleteDir ($path)
    {
        ...
        $folderName = end ($arrPath);
        $objFolder = new AppFolder();
        $arrFolder = $objFolder->getByName ($folderName);

        if ( is_dir ($path) )
        {
            $re = 'ok';
            if ( self::affectDir ($path, 'DEL_ROOT') == 0 )
                $re = 'E_CannotDelRoot';
            else
            {
        ...
        if ( $re === "ok" )
        {
            if ( $arrFolder !== false )
            {
                $objFolder->remove ($arrFolder[0]['FOLDER_UID'], $folderName);

                $documentVersion = new DocumentVersion();
                $arrDocuments = $documentVersion->getDocumentsForFolder ($arrFolder[0]['FOLDER_UID']);

                if ( $arrDocuments !== false )
                {
                    foreach ($arrDocuments as $arrDocument) {
                        $documentVersion->remove ($arrDocument['id']);
                    }
                }
            }
        }
    ...
    static public function moveDir ($path, $new_path)
    {
        if ( is_dir ($path) )
        {
            if ( self::affectDir ($path, 'MOVE_ROOT') == 0 )
                $re = 'E_CannotMoveRoot';
            else if ( mb_strpos ($new_path, $path) === 0 )
                $re = 'E_CannotMoveDirToChild';
            else
            {
                $new_path = fixPath ($new_path);
                if ( file_exists ($new_path . '/' . basename ($path)) )
                    $re = 'E_DirAlreadyExists';
                else if ( rename ($path, $new_path . '/' . basename ($path)) )
                    $re = 'ok';
                else
                {
                    $re = 'E_MoveDir';
    ...
    static public function copyDir ($path, $new_path)
    {
        if ( is_dir ($path) )
        {
            ...
            if ( !is_dir ($new_path) )
                mkdir ($new_path, octdec (DIRPERMISSIONS));
            ...
            for ($i = 0; $i < $nr_d; $i++) {
                ...
                if ( !is_dir ($dir) )
                {
                    if ( !@mkdir ($dir, octdec (DIRPERMISSIONS)) )
                        $re[] = getTL ('E_Create') . ': ' . $dirs[$i];
                }
            }

            // copy the files
            for ($i = 0; $i < $nr_f; $i++) {
                if ( !@copy ($path . $files[$i], $new_path . $files[$i]) )
                    $re[] = getTL ('E_Copy') . ': ' . $files[$i];
            }

            $re = (count ($re) == 0) ? ['ok'] : $re;
        }
        else
            $re = 'E_CopyDirInvalidPath';
    ...
    static public function renameDir ($path, $new_name)
    {
        try {
            if ( is_dir ($path) )
            {
                if ( self::affectDir ($path, 'RENAME_ROOT') == 0 )
                {
                    $re = 'E_CannotRenameRoot';
                }
                else
                {
                    ...
                    $objFolder = new AppFolder();
                    $arrPath = explode ('/', $path);
                    $arrFolder = $objFolder->getByName (end ($arrPath));

                    if ( $arrFolder !== false )
                    {
                        $folderUid = $arrFolder[0]['FOLDER_UID'];
                        $arrFolder[0]['FOLDER_NAME'] = $new_name;
                        $arrFolder[0]['FOLDER_UPDATE_DATE'] = 'now';
                        $objFolder->update ($arrFolder[0]);
                    }
    ...
    static public function downloadDir ($path)
    {
        ...
        if ( !class_exists ('ZipArchive') )
            $re = '<script>alert("' . addslashes (getTL ('E_ZipArchive')) . '");</script>';
        else
        {

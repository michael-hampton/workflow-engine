...
class StepDocument extends InputDocument
{
    ...
    public function create ($arrayData)
    {
        try {
            if ( $this->validate () )
            {
                ...
                $this->loadObject ($arrayData);
                $this->documentId = $this->save ();

                $this->saveStepDocument ();
            ...
            {
                 $sMessage = '';
                $aValidationFailures = $this->getArrValidationErrors ();

                foreach ($aValidationFailures as $message) {
                    $sMessage .= $message . '<br />';
                }
                throw (new Exception ('The document cannot be created!<br />' . $sMessage));
            }
        } catch (\Exception $e) {
            throw $e;
        }
    ...
    public function saveStepDocument ()
    {
        $this->objMysql->_insert ("workflow.step_document", array("step_id" => $this->stepId, "document_id" => $this->documentId));
    }
    ...
    public function update ($inputDocumentUid, $arrayData)
    {
        try {
            ...
            //Load InputDocument
            $inputDocument = new InputDocument();
            $inputDocument->setId ($inputDocumentUid);
            $inputDocument->loadObject ($arrayData);

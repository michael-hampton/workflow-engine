...
//            }
        ...
        if ( $re === "ok" )
        {
            $appDocument = new DocumentVersion();
            $arrDoc = $appDocument->getFileByName (basename ($path));

            if ( isset ($arrDoc[0]) && !empty ($arrDoc[0]) )
            {
                foreach ($arrDoc as $doc) {
                    $appDocument->remove ($doc['id']);
                }
            }
        }
    ...
    static public function moveFile ($path, $new_path)
    {
        ...
        if ( is_file ($path) )
        {
            ...
            else if ( rename ($path, fixPath ($new_path)) )
                $re = 'ok';
            else
            {
                $re = 'E_MoveFile';
    ...
    static public function renameFile ($path, $name)
    {
        if ( is_file ($path) )
        {
            ...
            {
                if ( rename ($path, dirname ($path) . '/' . FmFile::CleanupFilename ($name)) )
                {
                    $re = 'ok';
                }
                else
        ...
        if ( $re === "ok" )
        {
            ...
            {
                foreach ($arrDoc as $doc) {
                    $appDocument->update (array("APP_DOC_UID" => $doc['id'], "DOC_VERSION" => $doc['document_version'], "APP_DOC_FILENAME" => $name));
                }
            }
        }
    ...
    static public function getThumb ($path)
    {
        ...
        if ( $w && $h )
            FmImage::CropCenter ($path, null, $w, $h);
        else
            FmImage::Resize ($path, null, $w, $h);
    }
        ...
        if ( is_dir ($path) )
        {
            if ( !empty ($_FILES['files']) && is_array ($_FILES['files']['tmp_name']) )
            {
                ...
                foreach ($_FILES['files']['tmp_name'] as $k => $v) {
                    ...
                    if ( !FmFile::CanUploadFile ($filename) )
                        $errorsExt[] = $filename;
                    else if ( !move_uploaded_file ($v, $filePath) )
                        $errors[] = $filename;
                    if ( FmFile::IsImage ($filename) && (intval (MAX_IMAGE_WIDTH) > 0 || intval (MAX_IMAGE_HEIGHT) > 0) )
                        FmImage::Resize ($filePath, $filePath, intval (MAX_IMAGE_WIDTH), intval (MAX_IMAGE_HEIGHT));
                }
                if ( $errors && $errorsExt )
                    $re = 'E_UploadNotAll';
                else if ( $errorsExt )
